...
问题来了，`xvariable`虽然看上去数字，但存储的实际是字符串，需要做一个转换。
...
```r
p <- ggplot(data_m, aes(x=xvariable, y=value,color=variable,group=variable)) + geom_line() + theme(legend.position=c(0.1,0.8))
p
dev.off()
```

图终于出来了，调了下legend的位置，看上去还可以。

![]({{ site.img_url }}/splot/line_3.png)
	
有点难看，如果平滑下，会不会好一些，`stat_smooth`可以对绘制的线进行局部拟合。在不影响变化趋势的情况下，可以使用。
	
```r
p <- ggplot(data_m, aes(x=xvariable, y=value,color=variable,group=variable)) + geom_line() + stat_smooth(method="auto", se=FALSE) + theme(legend.position=c(0.1,0.8))
p
dev.off()
```	

从图中看，趋势还是一致的，主要是更好看了。

![]({{ site.img_url }}/splot/line_4.png)

```r
p <- ggplot(data_m, aes(x=xvariable, y=value,color=variable,group=variable)) + stat_smooth(method="auto", se=FALSE) + theme(legend.position=c(0.1,0.8))
p
dev.off()
```
