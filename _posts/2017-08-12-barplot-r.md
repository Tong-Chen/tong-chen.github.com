...
柱状图也是较为常见的一种数据展示方式，可以展示基因的表达量，也可以展示GO富集分析结果。
...
```r
p <- ggplot(data_m, aes(x=xvariable, y=value))
...
```r
# 获取平均值和标准差
data_m_sd_mean <- data_m %>% group_by(xvariable) %>% dplyr::summarise(sd=sd(value), value=mean(value))
...
```r
p <- ggplot(data_m_sd_mean, aes(x=xvariable, y=value)) + 
	geom_bar(stat="identity") +
    geom_errorbar(aes(ymin=value-sd, ymax=value+sd))
p
```

![](http://blog.genesino.com/images//splot/barplor_errorbar_1.png)

```r
p <- ggplot(data_m_sd_mean, aes(x=xvariable, y=value)) + 
	geom_bar(stat="identity", aes(fill=xvariable)) +
    geom_errorbar(aes(ymin=value-sd, ymax=value+sd), width=0.2, position=position_dodge(width=0.75))
p
```

![](http://blog.genesino.com/images//splot/barplor_errorbar_2.png)
...
p <- ggplot(data_m, aes(x=variable, y=value)) +
	geom_bar(stat="identity", position="stack", aes(fill=xvariable)) +
...
# position="fill" 展示的是堆积柱状图各部分的相对比例
# position="stack" 展示的是堆积柱状图的原始值
p <- ggplot(data_m, aes(x=variable, y=value, group=xvariable)) +
	geom_bar(stat="identity", position="stack", aes(fill=xvariable)) +
	geom_text(aes(label=value), position=position_stack(vjust=0.5))
p
```
...
p <- ggplot(data_m, aes(x=variable, y=value)) +
	geom_bar(stat="identity", position="fill", aes(fill=xvariable))
p
```
