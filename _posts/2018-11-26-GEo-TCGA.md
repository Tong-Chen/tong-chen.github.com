...
GEO是当今最大、最全的公共基因数据资源库，包括基因的表达、突变、修饰等信息，涵盖几乎所有的疾病，且单个实验检测样品数目较多。TCGA数据库包含`11,000`个病人的`33`种肿瘤的`7`个不同层面的基因数据 (包括基因表达、CNV，SNP，DNA甲基化，miRNA，外显子组等)和临床数据，意在解析癌症发生的分子接触、肿瘤的亚型和治疗靶点等。
